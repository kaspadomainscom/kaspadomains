...
| `proxy.ts` | CSP with per-request nonce, COOP/CORP and reporting headers; `connect-src` allows only live KNS/Supabase APIs, and exact hashes preserve the built Next fatal-error page without allowing arbitrary inline styles; HSTS stays in `next.config.ts` | ✅ |
